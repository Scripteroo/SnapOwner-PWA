      ...
      <div className="px-4 mt-4 space-y-3 max-w-lg mx-auto">
        ...
        <div className="animate-slide-up delay-4">
          <AccordionSection title="Sales History" icon={<TrendingUp className="w-4 h-4" />} badge={`${MOCK_SALES_HISTORY.length}`}>
            <div className="space-y-3">{MOCK_SALES_HISTORY.map((sale, i) => (<div key={i} className="flex items-center justify-between py-2 border-b border-lens-border last:border-0"><div><p className="text-[13px] font-medium text-lens-text">{sale.buyer}</p><p className="text-[11px] text-lens-secondary">{sale.date}</p></div><p className="text-[14px] font-semibold text-lens-text">{sale.price}</p></div>))}</div>
          </AccordionSection>
        </div>

        <div className="animate-slide-up delay-5">
          <AccordionSection title="Tax History" icon={<Receipt className="w-4 h-4" />}>
            <div className="space-y-3">{MOCK_TAX_HISTORY.map((tax, i) => (<div key={i} className="flex items-center justify-between py-2 border-b border-lens-border last:border-0"><div><p className="text-[13px] font-medium text-lens-text">{tax.year}</p><p className="text-[11px] text-lens-secondary">Assessed: {tax.assessed}</p></div><p className="text-[14px] font-semibold text-lens-text">{tax.tax}</p></div>))}</div>
          </AccordionSection>
        </div>

        <div className="animate-slide-up delay-5">
          <AccordionSection title="Liens & Mortgages" icon={<Landmark className="w-4 h-4" />} badge={`${MOCK_LIENS.length}`}>
            <div className="space-y-3">{MOCK_LIENS.map((lien, i) => (<div key={i} className="py-2"><div className="flex items-center justify-between"><p className="text-[13px] font-medium text-lens-text">{lien.type}</p><p className="text-[14px] font-semibold text-lens-text">{lien.amount}</p></div><p className="text-[11px] text-lens-secondary mt-0.5">{lien.lender} · {lien.date}</p></div>))}</div>
          </AccordionSection>
        </div>
        ...
        <div className="animate-slide-up delay-6">
          <AccordionSection title="Building Permits" icon={<HardHat className="w-4 h-4" />} badge={`${MOCK_PERMITS.length}`}>
            <div className="space-y-3">{MOCK_PERMITS.map((permit, i) => (<div key={i} className="py-2 border-b border-lens-border last:border-0"><div className="flex items-center justify-between"><p className="text-[13px] font-medium text-lens-text">{permit.type}</p><span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${permit.status === "Completed" ? "bg-lens-green/10 text-lens-green" : "bg-yellow-100 text-yellow-700"}`}>{permit.status}</span></div><p className="text-[11px] text-lens-secondary mt-0.5">{permit.number} · {permit.date}</p></div>))}</div>
          </AccordionSection>
        </div>
      </div>
